...
| `creator_profiles` and `deal_room_saves`  | Product; opt-in public creator identity and user-saved public collections    | Until the user removes the profile/save or deletes the account; public profiles expose only display name, avatar URL, bio, and public collections | Deleted through profile and Deal Room cascades                                       |
